...
- Learned LWR/NLWR history is conditional: it gets full ranking weight only when the measured AUTO baseline is clean and fast. A poor AUTO baseline removes the learned-route bonus instead of reinforcing the route that just performed badly.
...
- Multi-pass responses include `route_stability`, classifying each node as `exact_stable`, `path_stable`, `auto_policy_stable_path_dynamic`, `unstable`, `single_pass`, or `incomplete`, with candidate/path agreement and compact pass winners.
